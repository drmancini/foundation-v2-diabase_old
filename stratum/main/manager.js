...
const Manager = function(config, configMain) {
  ...
  this.handleShare = function(jobId, client, submission) {
    ...
    const headerDigest = Algorithms.x11.hash();
    const coinbaseDigest = Algorithms.sha256d.hash();
    const blockDigest = Algorithms.x11.hash();
    ...
    const shareMultiplier = Algorithms.x11.multiplier;
    const shareDiff = Algorithms.x11.diff / Number(headerBigInt) * shareMultiplier;
    const blockDiffAdjusted = job.difficulty * Algorithms.x11.multiplier;
