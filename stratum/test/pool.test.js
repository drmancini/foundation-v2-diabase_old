...
describe('Test pool functionality', () => {
  ...
  test('Test pool manager setup [9]', (done) => {
    ...
    pool.on('pool.log', (type, text) => {
      if (type !== 'debug') {
        ...
        expect(text).toBe('Submitted a primary block (Dash:1) successfully to Dash\'s daemon instance(s)');
        done();
      }
    });
    mockSetupDaemons(pool, () => {
  ...
  test('Test pool workers handling [12]', () => {
    ...
    worker2.times = 49.1;
    const expected = {
      'XbtkVnc9XRLhxfmafNkafderCWSsXYZJaM': { 'immature': 9570.00569668, 'generate': 0 },
      'XbtkVnc9XRLhxfmafNkafderCWSsXYZJaN': { 'immature': 429.99430332, 'generate': 0 }};
